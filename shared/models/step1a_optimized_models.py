...
class OptimizedHybridModel(nn.Module):
    ...
    def __init__(self, input_dim: int = 10, num_classes: int = 2, sequence_length: int = 10):
        ...
        reduced_sequence_length = input_dim // 4
    ...
    def forward(self, x):
        ...
        # For sequence data with shape (batch_size, sequence_length, features)
        # we need to permute to (batch_size, features, sequence_length) for CNN
        if len(x.shape) == 3:
            x = x.permute(0, 2, 1)  # (batch_size, features, sequence_length)
        elif len(x.shape) == 2:
            x = x.unsqueeze(1)  # (batch_size, 1, features) for tabular data
