...
class FactoryDatasite:
    ...
    def __init__(self, factory_name: str, port:int = 8080, hostname: str = "localhost", dashboard_url: str = "http://localhost:5000", dev_mode: bool = False, reset: bool = True, verbose: bool = False):
        ...
        threading.Thread(
            target=FactoryDatasite.send_heartbeat,
            args=(self.factory_name, self.dashboard_url, self.heartbeat_time),
            daemon=True
        ).start()
    ...
    def register_with_dashboard(self):
        ...
        try:
            ...
            self.logger.info("✅ Successfully registered with central dashboard.")
        except requests.RequestException as e:
            self.logger.error(f"❌ Failed to register with central dashboard: {e}")
    ...
    def send_heartbeat(factory_name, dashboard_url, heartbeat_time):
        while True:
            try:
                ...
                pass
            time.sleep(heartbeat_time)  # Send heartbeat every 120 seconds
