...
def create_federated_clients(client_data_map: Dict[str, Tuple], model_class, model_params: Dict,
                           ...
                           learning_rate: float = 0.001) -> List[FederatedClient]:
    """
    Create federated clients from data distribution map
    ...
    Returns:
        List of initialized FederatedClient instances
    """
    clients = []
    
    print(f"Creating {len(client_data_map)} federated clients...")
...
def load_model_specific_data(model_type='tabular'):
    ...
    # Import step1_data_utils
    try:
        import sys
        import os
        # Add shared directory to path
        shared_path = os.path.join(os.path.dirname(__file__), '..', '..', 'shared', 'utils')
        if shared_path not in sys.path:
            sys.path.insert(0, shared_path)
        from step1_data_utils import DataLoader
    except ImportError as e:
        print(f"ERROR: Could not import step1_data_utils: {e}")
        print("SOLUTION: Please ensure step1_data_utils.py is in shared/utils/")
    ...
    try:
        ...
        if sys.platform.startswith('win'):  # Windows
            data_dir = "D:\\Development\\pdm-fdl\\shared\\processed_data\\"
